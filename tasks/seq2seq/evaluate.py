...
def rouge_metric(predictions, labels, examples, metric="rouge-1", duplicate_rate=0.7, dataset='cnn_dm'):
    ...
    #metric_dict = {"rouge-1": "rouge-1", "rouge-2": "rouge-2", "rouge-l": "rouge-l"}
    ...
    for pred, ref in zip(pred_list, ref_list):
        ...
        if lang == 'zh-cn' or lang == 'ja':
            score = rouge.get_scores(' '.join(pred), ' '.join(ref)) 
        else:
            score = rouge.get_scores(pred, ref)
        scores.append(score)
    scores = [score[0][metric_dict[metric]]['f'] for score in scores]
    #print(scores)
    scores = sum(scores) / len(scores)
    """
